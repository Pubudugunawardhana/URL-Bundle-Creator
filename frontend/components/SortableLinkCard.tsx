...
export function SortableLinkCard({ link, onPlay, onDelete, statusDropdown }: SortableLinkCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-link bg-zinc-100 dark:bg-zinc-900 overflow-hidden">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img 
          src={link.thumbnail || `https://i.ytimg.com/vi/${link.youtubeId}/maxresdefault.jpg`} 
          alt={link.title}
          onError={(e) => {
            (e.target as HTMLImageElement).src = `https://i.ytimg.com/vi/${link.youtubeId}/hqdefault.jpg`;
          }}
          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
        
        {link.duration && link.duration !== '00:00' && (
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-bold text-zinc-900 dark:text-white text-[15px] mb-4 line-clamp-2 leading-snug group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors" title={link.title}>
          {link.title}
        </h3>
