...
export function SortableListItem({ link, onPlay, disabled }: SortableListItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    transition,
    isDragging,
  } = useSortable({ id: link._id, disabled });
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center h-full mt-1">
          <h4 className="text-sm font-semibold text-zinc-900 dark:text-white truncate mb-1 group-hover/btn:text-emerald-600 dark:group-hover/btn:text-emerald-400 transition-colors">{link.title}</h4>
          <div className="flex items-center gap-3">
            <span className={`text-[10px] px-2 py-0.5 rounded-md font-medium border ${link.status === 'Watched' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : link.status === 'Watching' ? 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20' : 'bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700'}`}>{link.status}</span>
            {link.duration && <span className="text-[10px] text-zinc-500 flex items-center gap-1"><Clock size={10} /> {link.duration}</span>}
          </div>
        </div>
      </button>
    </div>
