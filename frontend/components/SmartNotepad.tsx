...
export function SmartNotepad({ linkId, initialNotes, onSeek, onClose, onSave, getCurrentTime }: SmartNotepadProps) {
  ...
  const saveNotes = async () => {
    setIsSaving(true);
    try {
      await api.put(`/links/${linkId}/notes`, { notes });
      if (onSave) onSave(linkId, notes);
      setIsEditing(false);
    } catch (error) {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900 w-full animate-in slide-in-from-right duration-300">
      ...
      {isEditing && (
        <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 border-t border-emerald-200 dark:border-emerald-500/20 text-xs text-emerald-700 dark:text-emerald-400">
          <span className="font-semibold">Tip:</span> Type a time like <code className="bg-emerald-200 dark:bg-emerald-500/30 px-1 rounded">01:25</code> and it will become a clickable link to jump the link to that exact second!
        </div>
      )}
    </div>
  );
